...
fn writeRemainingFilesInEpub(builder: *@This()) !void {
    ...
    const navPagePath = contentFolderName ++ "/nav.xhtml";
    {
        ...
        const T = struct {
            ...
            pub fn onEntry(t: *@This(), fullPath: []const u8, dirTitle: ?[]const u8, depth: usize) !void {
                ...
                if (dirTitle) |partTitle| {
                    ...
                    const targetTitle = t.s.articleTocTitles.get(fullPath) orelse unreachable;
                    const targetPath = t.s.fileMapping.get(.{ .local = fullPath }) orelse unreachable;

                    try t.w.writeAll(
                        \\<li><a href="
                    );
                    try gen.writeRelativeUrl(t.w, targetPath, '/', t.relToPath, '/');
                    try t.w.writeAll(
                        \\">
                    );
            ...
            // toDpeth is exclusive.
            fn closeLists(t: *@This(), toDepth: usize) !void {
                while (t.lastDepth > toDepth) {
                    try t.w.writeAll(
                        \\</ol></li>
                        \\
                    );
        ...
        var t: T = .{
            .w = &wa.writer,
            .s = session,
            .relToPath = contentFolderName,
        };

        try session.articleDirEntries.?.iterate(&t);
        try t.closeLists(0);
