...
_dirPathToExMap: std.StringHashMap(*ConfigEx) = undefined,
...
pub fn init(allocator: std.mem.Allocator, stdout: std.fs.File.Writer, stderr: std.fs.File.Writer) AppContext {
    ...
    //ctx._dirPathToExMap = .init(ctx.arenaAllocator);
...
pub const getDirectoryConfigEx = @import("AppContext-config.zig").getDirectoryConfigEx;
