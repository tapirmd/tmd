...
test "tmd format" {
    for (exampleTmdContents) |tmdContent| {
        ...
        var wa: std.Io.Writer.Allocating = try .initCapacity(std.testing.allocator, 1 << 19);
        defer wa.deinit();
        ...
        var wa2: std.Io.Writer.Allocating = try .initCapacity(std.testing.allocator, 1 << 19);
        defer wa2.deinit();
